...
// A principal mudança: criamos uma função de fábrica
// que recebe as dependências e retorna um objeto
export default function createAuthMiddleware(UserRepository, PostRepository) {
  const middleware = {
    ...
    checkResponseOwnerOrAdmin: async (req, res, next) => {
      try {
        ...
        console.error("Erro ao verificar propriedade da resposta:", error);
        return res.status(500).json({
          ok: false,
